...
//         setStatus(fetchedTicket.status); // Set status from fetched ticket
//       } catch (error) {
...
import { Ticket } from '../types';
import { updateTicket, getTicketById } from '../apiServer';
import { checkUserRole } from '../apiServer'; // Import the method to check user role
    ...
    <div className="container mx-auto py-8">
      ...
      {ticket && (
        <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          {isAdmin && ( // Render status field only if user is admin
            <div className="mb-4">
              <label htmlFor="status" className="block text-gray-700 text-sm font-bold mb-2">
                Status
              </label>
